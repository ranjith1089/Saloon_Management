...
export default function Customers() {
  ...
  const { data: memberships } = useQuery({
    queryKey: ['memberships', 'active-only'],
    queryFn: async () => (await api.get('/memberships', { params: { active: 'true', limit: 500 } })).data.data,
  });

  const membershipByCustomer = useMemo(() => {
    const map = new Map<string, any>();
    (memberships || []).forEach((m: any) => map.set(m.customerId, m));
    return map;
  }, [memberships]);
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-medium text-gray-700">Membership</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {isLoading ? (
                ...
                data?.data?.map((c: any) => {
                  const m = membershipByCustomer.get(c.userId);
                  return (
                      ...
                      <td className="px-4 py-3">
                        {m ? (
                          <span
                            ...
                            title={`Expires ${new Date(m.endDate).toLocaleDateString()}`}
                          >
